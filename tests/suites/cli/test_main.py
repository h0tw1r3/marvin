...
from marvin.cli.main import app
...
def dummy_review_service(monkeypatch: pytest.MonkeyPatch, review_service: ReviewService):
    monkeypatch.setattr("marvin.cli.commands.run_review.ReviewService", lambda: review_service)
    monkeypatch.setattr("marvin.cli.commands.run_inline_review.ReviewService", lambda: review_service)
    monkeypatch.setattr("marvin.cli.commands.run_context_review.ReviewService", lambda: review_service)
    monkeypatch.setattr("marvin.cli.commands.run_summary_review.ReviewService", lambda: review_service)
    monkeypatch.setattr("marvin.cli.commands.run_inline_reply_review.ReviewService", lambda: review_service)
    monkeypatch.setattr("marvin.cli.commands.run_summary_reply_review.ReviewService", lambda: review_service)
...
def test_show_config_outputs_json(monkeypatch: pytest.MonkeyPatch):
    ...
    """
    monkeypatch.setattr(
        "marvin.cli.main.settings.model_dump_json",
        lambda **_: '{"debug": true}'
    )

    result = runner.invoke(app, ["show-config"])
    assert result.exit_code == 0
    assert "Loaded marvin configuration" in result.output
