...
from marvin.libs.config.llm.base import LLMPricingConfig, LLMConfigBase
...
def fake_pricing(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("marvin.config.settings.llm.meta.model", "gpt-4-test")
...
def test_calculate_no_pricing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr("marvin.config.settings.llm.meta.model", "unknown-model")
