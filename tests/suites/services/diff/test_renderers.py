# tests/suites/services/diff/test_renderers.py

import pytest

from marvin.libs.diff.models import (
    DiffFile,
    DiffHunk,
    DiffRange,
    DiffLine,
...
from marvin.services.diff import renderers, tools
...
def test_build_full_file_current(monkeypatch: pytest.MonkeyPatch, sample_diff_file: DiffFile) -> None:
    monkeypatch.setattr(
        "marvin.services.diff.renderers.read_snapshot",
        lambda *_, **__: "keep A\nkeep B\nadded me",
    )
